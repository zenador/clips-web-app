...
(deffunction ask-question (?question $?allowed-values)
   (bind ?answer (prompt ?question $?allowed-values))
   (debug (str-cat "response: " ?answer))
   (if (lexemep ?answer) 
       then (bind ?answer (lowcase ?answer)))
   (while (not (member ?answer ?allowed-values)) do
      (debug (str-cat "illegal response: " ?answer))
      (bind ?answer (prompt ?question $?allowed-values))
      (debug (str-cat "response: " ?answer))
      (if (lexemep ?answer) 
          then (bind ?answer (lowcase ?answer))))
...
(defrule system-banner ""
  (declare (salience 10))
  =>
  (alert "The Engine Diagnosis Expert System")
)
